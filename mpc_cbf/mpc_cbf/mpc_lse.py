...
class MPCCBFNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('max_steering_deg', 60.0)
        ...
        self.Q = np.diag([Q_x, Q_y, 0.0, 0.0])
        self.R = np.diag([R_v, R_delta])
        self.R_smooth = np.diag([R_smooth_v, R_smooth_delta])
        self.Q_t = np.diag([Q_x, Q_y, Q_theta, Q_theta])
        self.Q_terminal = self.Q_t * 10
    ...
    def _build_ocp(self):
        ...
        for k in range(self.N):
            ...
            err = ca.vertcat(
                xk[0] - self.p_Ref[0],
                xk[1] - self.p_Ref[1],
                ca.sin(xk[2]) - ca.sin(self.p_Ref[2]),
                ca.cos(xk[2]) - ca.cos(self.p_Ref[2])
            )
            cost += ca.mtimes([err.T, self.Q, err])
            ...
            # Add incentive to prefer forward motion
            # cost += 0.2 * ca.fmax(0, -uk[0])**2
        ...
        err_t = ca.vertcat(
            self.X[self.N][0] - self.p_Ref[0],
            self.X[self.N][1] - self.p_Ref[1],
            ca.sin(self.X[self.N][2]) - ca.sin(self.p_Ref[2]),
            ca.cos(self.X[self.N][2]) - ca.cos(self.p_Ref[2])
        )
    ...
    def control_loop(self):
        ...
        # Add condition to stop the control loop if the robot is at the goal, use target tolerance to determine if the robot is close enough to the goal
        if self._active_source in ["rviz", "rviz_hold"]:
            dx = self._rviz_goal[0] - self.current_state[0]
            dy = self._rviz_goal[1] - self.current_state[1]
            dist = math.sqrt(dx * dx + dy * dy)
            if dist < self.goal_tolerance:
                self.get_logger().info("Goal reached, stopping control loop")
                self.publish_cmd(np.zeros(self.nu))
                return
        ...
        try:
            ...
            # Change initial guess to current state to help solver recover next time
            self.init_guess_x = np.tile(self.current_state.reshape(-1, 1), (1, self.N + 1))
            self.init_guess_u = np.zeros((self.nu, self.N))
            try:
                u_debug = np.array(
